...
class GoogleBigQuery:
    ...
    def buildSQL(self):
        sqlTxt = '''
           SELECT IFNULL(FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', TIMESTAMP_SECONDS(SAFE_CAST(visitStartTime+hits.time/1000 AS INT64)), 'America/Denver'), '\000') as hit_timestamp,
                ...
                FROM `<bq_table>_<processDate>`, UNNEST(hits) AS hits
        '''
    ...
    def getFrmBQ(self, sql:str):
        ...
        try:
            ...
            self._log.error("Failed to find the Table <bq_table>_{}".format(self.__prcDtStr))
...
python3 GoogleBigQuery.py \
 --tempS3 "s3://prod-abc-datalake-temp-us-east-1/ga_load/athena_res" \
 --encryptionType SSE_KMS \
 --kmsKeyArn "arn:aws:kms:us-east-1:xxxxx:key/xxxxx-d3fa-4f91-be18-xxxxx" \
 --awsRole "arn:aws:iam::xxxxx:role/Prod-BigDataETLProdSupp"
 -useSAML

python GoogleBigQuery.py --tempS3 "s3://cloudera-migration/temp/ga_load/athena_res"  --encryptionType SSE_KMS  --kmsKeyArn "arn:aws:kms:us-east-1:yyyyyyy:key/ddddd-dee1-47bd-9417-nnnnnnnn"  --awsRole "arn:aws:iam::yyyyyyy:role/Dev-BigDataPowerDeveloper" -useSAML
